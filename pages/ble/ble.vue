...
export default {
  ...
  methods: {
    ...
    hex2str(str = 'c7e0b5babaa3b6fbc9faceefd2bdc1c634') {
      // 53562d312e302e34
      ...
      for (var i = 0; i < str.length; i = i + 2) {
        ...
        string.push(String.fromCharCode(parseInt(str.substr(i, 2), 16)))
      }
      console.log(ble.decode(gbks))
      return string.join('')
    },
    moveHandle() {},
    /**
    ...
    getBluetoothDevices() {
      uni.getBluetoothDevices({
        success: res => {
          ...
          this.list = res.devices.filter(v => v.localName)
      ...
      uni.getBLEDeviceCharacteristics({
        ...
        success: res => {
          ...
          for (let j in res.characteristics) {
            console.log(res.characteristics[j].properties.notify)
            // if (!res.characteristics[j].properties.notify) {
            //   return
            // }
            if (res.characteristics[j].properties.notify) {
              console.log('启用notify：' + res.characteristics[j].uuid)
              uni.notifyBLECharacteristicValueChange({
                ...
                success(res_notify) {
                  console.log('启用notify成功：' + JSON.stringify(res_notify))
                }
              })
            }
          }
    ...
    writeBLECharacteristicValue() {
      ...
      // const deviceId = '14:0B:02:11:16:09'
      ...
      const codes = [0x53, 0x4e, 0x44, 0x01, 0x55, 0x36, 0x9e]
      // const codes = [0x53, 0x4e, 0x44, 0x02, 0x55, 0x36, 0x6e]
      const _queue = Array.from(commands.GET_UNIT_NAME)
      const buffer = new Uint8Array(_queue).buffer

      ble.writeBLECharacteristicValue({ deviceId, serviceId, characteristicId, buffer }).then(res => {
    ...
    batchNotify() {
      let deviceId = this.equipment[0].deviceId
      console.log('开始batchNotify')
      // 获取所有service
      uni.getBLEDeviceServices({
        // 这里的 deviceId 需要已经通过 createBLEConnection 与对应设备建立链接
        deviceId,
        success: res1 => {
          console.log(JSON.stringify(res1.services))
          console.log('获取设备服务成功:' + res1.errMsg)
          this.$set(this.disabled, 7, true)
          this.$set(this.disabled, 8, true)
          this.showMaskType = 'service'
          this.list = res1.services

          this.characteristicsData = []
          if (res1.services.length <= 0) {
            toast('获取服务失败，请重试!')
            return
          }
          this.maskShow = true
          // 获取所有特征值
          for (let i in res1.services) {
            uni.getBLEDeviceCharacteristics({
              deviceId,
              serviceId: res1.services[i].uuid,
              success: res2 => {
                console.log(JSON.stringify(res2))
                console.log('获取特征值成功:' + res2.errMsg)
                this.valueChangeData = {}
                if (res2.characteristics.length <= 0) {
                  toast('获取特征值失败，请重试!')
                  return
                }
                for (let j in res2.characteristics) {
                  if (!res2.characteristics[j].properties.notify) {
                    return
                  }
                  console.log('启用notify' + j + ':' + res1.services[i].uuid + ':' + res2.characteristics[j].uuid)
                  uni.notifyBLECharacteristicValueChange({
                    state: true, // 启用 notify 功能
                    deviceId,
                    serviceId: res1.services[i].uuid,
