...
### 1.1 全局配置修改（在request/index.js文件中修改config和interceptor）
``` javascript 
	config: {
...
### 1.2 具体接口调用时修改（在request/index.js文件中具体业务接口中配置)
``` javascript
//设置baseUrl
	...
	export default {
		...
		methods: {
			...
			sendRequest: function() {
				this.loading = true
				this.$api.test({noncestr: Date.now(), loading: true}).then((res)=>{
					this.loading = false;
			...
			async sendRequest1() {
				this.loading = true
				let res = await this.$api.test({noncestr: Date.now(), loading: true});
				this.loading = false;
				this.res = '请求结果 : ' + JSON.stringify(res);
			}
		}
...
在request/index.js文件中的request(Object)方法中补充修改相应的代码
...
在api/index.js文件中的编写具体业务相关的接口,参考test()方法
